...
async function createTask(token: string, query: string) {
  const formData = {
    prompt: query,
    fieldSort: 1,
    taskId: `bench-${crypto.randomUUID()}`,
    fileId: [0],
...
async function pollTask(token: string, taskId: string) {
  ...
  while (Date.now() - started < TIMEOUT_MS) {
    ...
    const row = rows.find((r: any) => Number(r.sort) === 1) || rows[0];
...
async function run() {
  ...
  const results: Array<{ query: string; status: string; totalMs: number; ttftMs: number | null; contentLen: number }> = [];
  ...
  const ttfts = done.map((r) => r.ttftMs).filter((v): v is number => typeof v === 'number');

  const summary = {
    completed: done.length,
    total: results.length,
    statusCounts: results.reduce((acc: Record<string, number>, r) => {
