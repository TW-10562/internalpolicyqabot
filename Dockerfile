...
filtered = [line for line in src if not re.match(r'^\\s*torch(\\s|==|>=|<=|$)', line)]
...
COPY config /app/config
EXPOSE 8010
CMD ["bash","-lc","python -c \"from services.gpu_guard import ensure_cuda_or_raise; ensure_cuda_or_raise('rag-api-startup')\" && uvicorn api.main:app --host 0.0.0.0 --port 8010"]
